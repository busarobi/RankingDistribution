...
for phi0 in [0.1, 0.5, 0.7]:
    ...
    #plt.show()
    plt.xlim([np.max([0.0, phi0-0.05]),np.min([phi0+0.05, 1.0])])
    plt.ylim([-0.01,1.01])
    ...
    plt.tight_layout()
    # plt.show()
    pdf_fanme =  '/Users/busafekete/Dropbox/Apps/Overleaf/2018-RankHyp/testingMallows/figs/finite_conf_%g_m_%d' % (phi0,m)
