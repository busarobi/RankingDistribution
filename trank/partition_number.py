...
class Partition:
    ...
    def set_params(self):
        ...
        if exists:
            ...
            for i in range(1, self.upperbound):
...
class SufficientStatisticDistribution:
    def __init__(self, N, n, phi, reload = True):
        self.name = "Sufficient statistic distribution"
        self.reload = reload
        self.N = N # number of items
        self.n = n # number of samples
        self.m = (self.N * (self.N - 1)) / 2 # max Kendall distance
        self.upperbound = self.n * self.m + 1 # upper bound for the domain
        self.mahonian = Mahonian(self.N)
        self.phi = phi
        self.norm = normalizationMallows(self.N, self.phi)
        self.set_params()
    ...
    def set_params(self):
        ...
        dir_path = os.path.dirname(os.path.realpath(__file__))
        fname = dir_path + ('/partition/data_approx_item_%d_sample_size_%d_phi_%g.txt' % (self.N, self.n, self.phi))
        exists = os.path.isfile(fname)
        if (exists & self.reload):
            ...
            with open(fname, 'r') as f:
                for i, line in enumerate(f):
                    self.arr[i] = float(line)
        else:
            #print(self.upperbound)
            pi0 = range(1, self.N + 1)
            mallows = Mallows(pi0, self.phi)
            normalization = np.power(mallows.get_normalization(), self.n)
            #self.norm_mahonian = [self.mahonian.get(i)/normalization for i in range(self.mahonian.m)]


            self.memo = [[0 for i in range(self.upperbound)] for j in range(self.n+1)]
            for i in range(1,self.n+1): self.memo[i][0] = 1/normalization
            for i, v in enumerate(self.mahonian.getArray()): self.memo[1][i] = v/normalization
            # for i in range(1,self.n+1): self.memo[i][0] =  - n*np.log(mallows.get_normalization())
            # for i, v in enumerate(self.mahonian.getArray()): self.memo[1][i] = np.log(v) - self.n * np.log(mallows.get_normalization())
            ...
            # self.arr = [self.memo[self.n][i] for i in range(self.upperbound)]
            # self.arr[self.upperbound-1]=1
            self.arr = np.zeros((self.upperbound,))
            for i in range(self.upperbound):
                self.arr[i] = np.power(self.phi,i) * self.memo[self.n][i] #/ normalization
            self.arr[self.upperbound-1] = np.power(self.phi,self.upperbound-1)#/ normalization
...
if __name__ == "__main__":
    ...
    for i in range(4,10):
        for n in range(2, 40):
            ...
            stat_dist = SufficientStatisticDistribution(i,n, phi, False)
            ...
            s = 0.0
            for ind, v in enumerate(stat_dist.getArray()):
                s += ind * v
